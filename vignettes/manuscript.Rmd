---
title: 'Example Manuscript: A template for using and R package to host a manuscript'
author: 
...
- affilnum: 2
  affil: Big Name University, Department of #Rstats, City, BN, 01020, USA
- affilnum: 3
...
Suspendisse elit neque, placerat vitae venenatis id, auctor vestibulum augue. Vivamus iaculis magna at sapien sodales, a sagittis tellus sagittis. Sed laoreet ac massa id fringilla. In et enim eget ante tincidunt aliquet ut in risus(Figure \ref{tab:Table1}). In vestibulum, nisl non viverra ullamcorper, odio nisl scelerisque sapien, vitae ornare neque odio ut odio. Maecenas vitae leo rhoncus, egestas quam ac, dapibus eros. 
```{r Table1, results='asis', echo=FALSE, tab.cap="This is the first example table\\label{tab:Table1}",cache=TRUE}
kable(head(iris))
```
...
```{r Fig1, echo=FALSE, fig.cap="Just my first figure with a very fantastic caption. \\label{fig:Fig1}",cache=TRUE}
x<-rnorm(100)
y<-jitter(x,1000)
plot(x,y)
```

```{r Table2, echo=FALSE,tab.cap="A second table showing some of the mtcars dataset.  \\label{tab:Table2",cache=TRUE}
kable(mtcars[1:12,])
```
...
```{r Fig12, echo=FALSE, fig.cap="Second figure showing a boxplot with ground breaking results. \\label{fig:Fig2}",cache=TRUE}
a<-sort(rnorm(100))
...
Nullam diam quam, egestas sed erat quis, placerat tincidunt nisl. Sed varius ac tortor eget fermentum. Sed ac magna tellus. Cras dolor felis, gravida eu lacus eget, convallis efficitur lacus. In vulputate neque quis eros consectetur, sit amet vehicula quam consequat. Nulla euismod quis lorem sit amet dapibus. Nam hendrerit ante et leo rutrum dignissim. Maecenas semper nec magna quis aliquam. Vivamus urna purus, lobortis ut imperdiet a, pretium in mauris.
